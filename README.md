...
AI 화상 프로젝트를 하면서 한 가지가 계속 걸렸다.

AI는 항상 기다린다. 사용자가 먼저 말을 걸어야만 움직인다.
그런데 사람은 자기가 뭘 필요로 하는지 항상 알고 있지 않다.
모르기 때문에 묻지 못하고, 묻지 않으면 AI는 아무것도 하지 않는다.

나는 반대로 작동하는 AI를 만들고 싶었다.
사용자의 행동 패턴과 상태를 보다가, 필요한 순간에 **먼저 말을 거는 것.**
사용자가 불필요하게 입력하지 않아도 되는, 그런 AI.

Vela는 그 생각의 연장선이다. 아직 만들어가는 중이다.
...
일반 생성형 AI는 **반응형**이다.  
사용자가 먼저 물어야 답한다. 뭘 물어야 할지 모르면, AI는 침묵한다.

Vela는 백그라운드에서 데이터를 보다가 **먼저 말을 건다.**  
도메인은 바꿔도, 구조는 하나다.
